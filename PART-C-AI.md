...
The way the business rules were separated into their own functions. The code was simple and the rules are easy to understand. I read through the code and tested different cases including the invalid status changes and sending a request without a resolution note. Everything was working as expected that's why i kept it without changing anything.